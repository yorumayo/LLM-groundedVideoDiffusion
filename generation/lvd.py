...
def init(base_model):
    ...
    unet = UNet3DConditionModel.from_pretrained(model_key, subfolder="unet").to(
        torch.float16
    )
    pipe = TextToVideoSDPipeline.from_pretrained(
        model_key, unet=unet, torch_dtype=torch.float16
    )
